...
class DiagonaledMM(torch.autograd.Function):
    ...
    def _compile_function(dtype: str, device: str, b0: int = 4, b1: int = 4, b2: int = 16):
        ...
        c = tvm.var('c')  # hidden dimension
        w = tvm.var('w')  # bucket size
        padding = tvm.var('padding')  # padding
        X = tvm.placeholder((b, n, h, c), name='X', dtype=dtype)  # query
        k = tvm.reduce_axis((0, c), name='k')  # dimension to sum over
        D = tvm.placeholder((b, n, h, h), name='D', dtype='bool')  # attention mask
        output_shape = (b, n, h, h)  # shape of the result tensor
        algorithm = lambda l, i, q, j: tvm.sum(
                tvm.if_then_else(
                    D[l][i][q][j] == True,
                    X[l, i, q, k] * X[l, i, j, k] ,  # t1 is diagonaled
                    padding
                ),
                axis=k)
        ...
        print('Lowering: \n ===================== \n{}'.format(tvm.lower(s, [X, D], simple_mode=True)))
        ...
        diagonaled_mm = tvm.build(s, [X, Z, D, w, padding], target=device, target_host=tgt_host, name='diagonaled_mm')
