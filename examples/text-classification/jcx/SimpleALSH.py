...
class SimpleALSHAttention16(nn.Module):
    ...
    def forward(self, qk_, attention_mask_, bucket_size=32, **kwargs):
        ...
        tmp_zeros = torch.zeros(qk_const.shape, device=device, dtype=torch.float16)
        q = torch.cat((qk_norm, tmp_zeros, qk_const), -1)
        p = torch.cat((qk_norm, qk_const, tmp_zeros), -1)
