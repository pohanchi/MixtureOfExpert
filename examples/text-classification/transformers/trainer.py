...
class Trainer:
    ...
    def __init__(
        ...
        if is_wandb_available() and not self.args.disable_wandb:
            self._setup_wandb()
        else:
            logger.info(
    ...
    def _setup_wandb(self):
        ...
        wandb.init(project=os.getenv("WANDB_PROJECT", "huggingface"), config=vars(self.args),name=self.args.wandb_name)
        # keep track of model topology and gradients
        if os.getenv("WANDB_WATCH") != "false":
            wandb.watch(
                self.model, log=os.getenv("WANDB_WATCH", "all"), log_freq=max(100, self.args.logging_steps)
            )
